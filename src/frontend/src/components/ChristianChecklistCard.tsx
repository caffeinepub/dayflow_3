...
const ITEMS_KEY = "dayflow_christian_items";
const DONE_KEY = (dateKey: string) => `dayflow_christian_done_${dateKey}`;
...
export default function ChristianChecklistCard() {
    ...
    <div className="rounded-2xl border border-white/[0.06] bg-df-navy-mid shadow-card card-glow flex flex-col h-full min-h-[340px] p-5">
      ...
      <div className="flex items-center gap-2 mb-4">
        <div
          ...
          style={{ background: "oklch(0.78 0.15 80 / 0.15)" }}
        >
          <Heart className="w-4 h-4" style={{ color: "oklch(0.78 0.15 80)" }} />
        </div>
        <h3 className="text-sm font-semibold text-df-text">Daily Faith</h3>
        ...
        <AnimatePresence initial={false}>
          {items.length === 0 && (
            <motion.div
              ...
              data-ocid="faith.empty_state"
            >
              Add your first faith practice below.
            </motion.div>
          )}
          {items.map((item, i) => (
            <motion.div
              ...
              data-ocid={`faith.item.${i + 1}`}
            >
              <button
                type="button"
                onClick={() => toggleItem(item)}
                data-ocid={`faith.checkbox.${i + 1}`}
              ...
              <button
                ...
                data-ocid={`faith.delete_button.${i + 1}`}
